...
export function ProposalDisplay({ front, back, status, onAccept, onEdit, onReject }: ProposalDisplayProps) {
  ...
  return (
    <Card className={isRejected ? "opacity-50" : ""}>
      ...
      <CardFooter className="flex flex-wrap gap-2">
        <Button variant="default" size="sm" onClick={onAccept} disabled={isAcceptedOrEdited}>
          {isAcceptedOrEdited ? "Accepted" : "Accept"}
        </Button>
        <Button variant="outline" size="sm" onClick={onEdit} disabled={isRejected}>
          Edit
        </Button>
        <Button variant="destructive" size="sm" onClick={onReject} disabled={isRejected}>
