...
export function ProposalDisplay({
  ...
  onReject,
  index,
}: ProposalDisplayProps & { index: number }) {
    ...
    <Card className={isRejected ? "opacity-50" : ""} data-test-id={`proposal-card-${index}`}>
      <CardHeader>
        <div className="flex items-start justify-between gap-2">
          <Badge variant={getBadgeVariant()} data-test-id={`proposal-status-badge-${index}`}>
            {getStatusLabel()}
          </Badge>
        </div>
      </CardHeader>
      <CardContent className="space-y-4">
        <div>
          ...
          <p className="text-base font-medium" data-test-id={`proposal-front-text-${index}`}>
            {front}
          </p>
        </div>
        <div>
          <h4 className="mb-2 text-sm font-semibold text-muted-foreground">Back</h4>
          <p className="text-sm" data-test-id={`proposal-back-text-${index}`}>
            {back}
          </p>
        </div>
      </CardContent>
      <CardFooter className="flex flex-wrap gap-2">
        <Button
          ...
          data-test-id={`proposal-accept-button-${index}`}
        >
        ...
        <Button
          ...
          onClick={onEdit}
          disabled={isRejected}
          data-test-id={`proposal-edit-button-${index}`}
        >
          Edit
        </Button>
        <Button
          ...
          onClick={onReject}
          disabled={isRejected}
          data-test-id={`proposal-reject-button-${index}`}
        >
